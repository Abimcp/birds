...
function deleteBird(id, li) {
  ...
  fetch(`http://localhost:3000/bird/${id}`, options)
    .then(li.remove())
    .catch(console.warn);
}

// helpers
function appendBird(data) {
  data.dogs.forEach(appendBird);
}

function appendBird(birdData) {
  const newRow = document.createElement("tr");
  const birdLi = formatDogTr(birdData, newRow);
